...
public class UITSlider : VisualElement
{
    ...
    /// <summary>
    /// 内部 Slider のトラック要素（unity-base-slider__tracker）の worldBound（panel 座標）を返す。
    /// 取得できない/幅 0 のときは false。レーザーで panelX → 値へ写像する際の基準矩形に使う。
    /// </summary>
    public bool TryGetTrackWorldBound(out Rect worldBound)
    {
        worldBound = default;
        if (m_slider == null) return false;
        var tracker = m_slider.Q(className: "unity-base-slider__tracker");
        worldBound = tracker != null ? tracker.worldBound : m_slider.worldBound;
        return worldBound.width > 0f;
    }

    /// <summary>正規化値(0..1)で値を設定する。OnValueChanged が発火し既存の Accessor 反映・保存が走る。</summary>
    public void SetNormalized(float n01)
    {
        if (m_slider == null) return;
        float v = m_slider.lowValue + Mathf.Clamp01(n01) * (m_slider.highValue - m_slider.lowValue);
        m_slider.value = v; // RegisterValueChangedCallback → OnValueChanged
    }
